...
let app = {
    ...
    initializeMap : function() {
        this.map = new GoogleMap(document.getElementById("map"), "#000000");
    },
    ...
    registerEvents : function() {
        ...
        document.getElementById("geolocation-show").addEventListener("click", ()=>{
            this.map.setMarker(this.currentposition,document.getElementById('geolocation-text').value);
        });

        navigator.geolocation.watchPosition((position)=>{
            position = convertGeoLocationToPlainObject(position);
            this.currentposition = position;
            document.getElementById("geolocation-coordinate").innerHTML = this.geoPositionToString(position);
        },(error) => console.log(error.message),{enableHighAccuracy: true});
    },
